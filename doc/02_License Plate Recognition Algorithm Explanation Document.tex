...
\begin{document}
	...
	\begin{itemize}
		\item Channel 1: Grayscale image.
		\item Channel 2: RGB555 or RGB565, often used in program processing such as Fourier transforms.
		\item Channel 3: RGB color image.
		\item Channel 4: RGBA (RGB plus an alpha channel for transparency, often found in PNG images).
	\end{itemize}
	
	\subsection{CvType Type Constant Combination}
	\begin{itemize}
		\item Byte refers to the number of bits (8, 16, 32, or 64 bits).
		\item U, S, and F represent unsigned int, signed int, and float types respectively.
		\item C refers to the number of channels. For example, \texttt{CV\_8UC3} is an 8-bit unsigned 3-channel image.
	\end{itemize}
